...
class Related_Posts_Widget extends WP_Widget {
    function __construct() {
        parent::__construct(
            ...
            array('description' => __('A widget to display related posts based on tags', 'text_domain'),) // Args
        );
    }
...
//end related tag post widget
//
//tag for specific article
// Function to display a post by ID
function display_post_by_id($atts) {
    // Extract the attributes passed to the shortcode
    $atts = shortcode_atts(
        array(
            'id' => '', // Default value if no ID is provided
        ),
        $atts,
        'post_by_id'
    );
    ...
    if ($post) {
        ...
        return '<h2>' . esc_html($post->post_title) . '</h2>' . '<div>' . apply_filters('the_content', $post->post_content) . '</div>';
    } else {
...
add_shortcode('post_by_id', 'display_post_by_id');
